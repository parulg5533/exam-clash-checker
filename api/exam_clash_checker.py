...
class ExamClashChecker:
    def __init__(self, reg_data_path: str = None, timetable_path: str = None):
        ...
        # Fast load from precompiled cache if available
        loaded_from_cache = False
        if reg_data_path is None:
            try:
                try:
                    import data_cache
                except ImportError:
                    from . import data_cache
                self._load_from_cache(data_cache.DATA)
    ...
    def _load_from_cache(self, data: dict):
        """Loads precomputed student-course mappings and timetable slots."""
        for roll, courses in data["students"].items():
            self.student_to_courses[roll] = set(courses)
            for c in courses:
                self.course_to_students[c].add(roll)

        for code, cdata in data["courses"].items():
            self.course_info[code] = CourseInfo(
                code=code,
                name=cdata["name"],
                enrolled_count=cdata["enrolled_count"],
                students=set(cdata["students"]),
            )
        ...
        for s in self.slots:
            for c in s["courses"]:
                self.course_to_slots[c].append(s)
